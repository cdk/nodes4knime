...
/**
 * This class is the model for the CDK 2D-generation node. It takes the input
 * molecules (if there are any), checks if they already have 2D coordinates
 * assigned and if not creates a new copy of the molecule and computes 2D
 * coordinates for it. The columns with molecules will have a property
 * afterwards, that indicates that 3D coordinates are available (
...
public class Coord2DNodeModel extends ThreadedColAppenderNodeModel {
    ...
    private static final NodeLogger LOGGER = NodeLogger
            .getLogger(Coord2DNodeModel.class);
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
        ...
        if (molColIndex == -1) {
            ...
            for (DataColumnSpec spec : inSpecs[0]) {
                if (spec.getType().isCompatible(CDKValue.class)) {
                    if (molColIndex != -1) {
                        setWarningMessage("Column '" + spec.getName()
                                + "' automatically chosen as molecule column");
                    }
                    molColIndex = i;
        ...
        if (!inSpecs[0].getColumnSpec(molColIndex).getType()
                .isCompatible(CDKValue.class)) {
            throw new InvalidSettingsException("Column '" + m_colName
                    + "' does not contain CDK cells");
        }
    ...
    private static DataColumnSpec createSpec(final DataColumnSpec oldSpec) {
        ...
        DataColumnProperties props =
                oldSpec.getProperties().cloneAndOverwrite(PROP_2D);
    ...
    protected ExtendedCellFactory[] prepareExecute(final DataTable[] data)
            throws Exception {
        final int colIndex =
                data[0].getDataTableSpec().findColumnIndex(m_colName);
        if (colIndex == -1) {
            throw new InvalidSettingsException("Column '" + m_colName
                    + "' does not exist in input table");
        }
        if (!data[0].getDataTableSpec().getColumnSpec(colIndex).getType()
                .isCompatible(CDKValue.class)) {
            throw new InvalidSettingsException("Column '" + m_colName
                    + "' does not contain CDK cells");
        }

        final TimeoutThreadPool pool = new TimeoutThreadPool("Coord2D");
        ...
        ExtendedCellFactory cf = new ExtendedCellFactory() {
            ...
            public DataCell[] getCells(final DataRow row) {
                ...
                if (hasCoords) {
                    ...
                    try {
                        final Pointer<IMolecule> pClone =
                        ...
                        pool.run(r, 5000); // allow max 5 sec.
                        // ...]added to stop endless runs
                        if (pClone.get() != null) {
            ...
            public DataColumnSpec[] getColumnSpecs() {
                return new DataColumnSpec[]{createSpec(data[0]
                        .getDataTableSpec().getColumnSpec(colIndex))};
            }
        };
