...
public class SSSearchNodeModel extends ThreadedTableBuilderNodeModel {
     ...
     */
    @Override
    protected DataTableSpec[] prepareExecute(final DataTable[] data)
            throws Exception {
        m_columnIndex =
                data[0].getDataTableSpec().findColumnIndex(
                        m_settings.molColName());

        return new DataTableSpec[]{data[0].getDataTableSpec(),
                data[0].getDataTableSpec()};
    }
     ...
     */
    @Override
    protected void processRow(final DataRow inRow,
        ...
        if (inRow.getCell(m_columnIndex).isMissing()) {
            ...
            IAtomContainer mol =
                    ((CDKValue)inRow.getCell(m_columnIndex)).getAtomContainer();
        ...
        if (m_settings.molColName() == null) {
            for (DataColumnSpec dcs : inSpecs[0]) {
                if (dcs.getType().isCompatible(CDKValue.class)) {
                    if (m_settings.molColName() != null) {
                        setWarningMessage("Using '" + m_settings.molColName()
                                + "' as molecule column");
                        break;
                    } else {
     ...
     */
    private static IAtomContainer createMolecule(final String... smiles)
            throws CDKException {
        if ((smiles != null) && (smiles.length > 0)) {
            SmilesParser parser =
                    new SmilesParser(
                            NoNotificationChemObjectBuilder.getInstance());
            IMolecule mol = parser.parseSmiles(smiles[0]);
            CDKHueckelAromaticityDetector.detectAromaticity(mol);
            return mol;
        } else {
            return NoNotificationChemObjectBuilder.getInstance().newInstance(
