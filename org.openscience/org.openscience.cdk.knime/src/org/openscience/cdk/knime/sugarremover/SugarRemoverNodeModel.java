...
import org.knime.chem.types.SmilesValue.SmilesUtilityFactory;
...
import org.openscience.cdk.knime.hydrogen.HydrogenAdderSettings;
...
public class SugarRemoverNodeModel extends NodeModel {
    ...
    private static final NodeLogger logger = NodeLogger
            .getLogger(SugarRemoverNodeModel.class);
        
    
    private final SugarRemoverSettings m_settings =
        new SugarRemoverSettings();
        	...
        	if (inRow.getCell(molColIndex).isMissing()){
        		...
        		if(newMols.isEmpty()){
        			...
        			for (int i=0; i < newMols.size(); i++){
        				...
        				if(!explicitH_flag)
        					newMols.set(i,new CDKCell(normalizer.convertExplicitToImplicitHydrogens(
        								newMols.get(i).getAtomContainer())));
        				DataCell[] newRow = buildRow(molColIndex, tbspecs,
        						inCells, newMols.get(i));
     ...
     * @return
     */
	private DataCell[] buildRow(final int molColIndex, DataTableSpec tbspecs,
			DataCell[] inCells, CDKCell newMol) {
		DataCell[] newRow =  new DataCell[tbspecs.getNumColumns()];
		...
		if(m_settings.replaceColumn()){
			...
			newRow[tbspecs.findColumnIndex(m_settings.appendColumnName())] = newMol==null?DataType.getMissingCell():newMol;
		}
		return newRow;
	}
    ...
    private List<CDKCell> removeSugars(IAtomContainer atomContainer) {
		...
		for (int i = 0 ; i < ringset.getAtomContainerCount(); i++) {
			...
			// get the the bond order of all the atoms in the ring (atom container)
			// bonds outside of the ring are not considered
			IBond.Order bondorder = AtomContainerManipulator.getMaximumBondOrder(one_ring);
			// if all bonds between the atoms in the ring are single and the rings have formula below 
			if (IBond.Order.SINGLE.equals(bondorder) & (formula.equals("C5O") | formula.equals("C4O") | formula.equals("C6O"))) {
	...
	private boolean hasGlycosideBond(IAtomContainer ring, IAtomContainer molecule, IRingSet ringset) {
		...
		// if there are no other sugar rings connected and the atoms in the ring do not have any double bond
		
		if (IBond.Order.SINGLE.equals(BondManipulator.getMaximumBondOrder(bonds)) && connectedRings.getAtomContainerCount() == 0) {
			// get connected atoms of atoms in sugar ring to check if there is a glycoside bond
			// gycoside bond: a bond is established between one of the oxygens connected to the ring and a carbon from a substituent group
			for (IAtom atom : sugarRing.atoms()) {
			...
			for (IAtom connected_atom : connectedAtoms) {
				...
				if (!sugarRing.contains(connected_atom) && connected_atom.getSymbol().matches((new Atom("O").getSymbol()))) {
					...
					for(IAtom neighbour : conn_Atoms){
						...
						// check if the carbon is not from the ring and is a carbon from a substituent group
						c_other = c_other || (!sugarRing.contains(neighbour) && !neighbour.getSymbol().matches(new Atom("H").getSymbol()));
					}
...
/**
 * this method returns all the a glycoside bond in the iatomcontainer attaching the oxygen to the carbon in the sugar ring 
 * @param ring
 * @param molecule
 * @param ringset
 * @return
 */
        ...
        if(!m_settings.replaceColumn()){
        	...
        	if (name == null || name.length() == 0) {
        		throw new InvalidSettingsException(
        				"Invalid name for appended column");
        	}
    ...
    private DataColumnSpec[] createColSpec(final DataTableSpec in) {
    	DataColumnSpec[] colSpecs = new DataColumnSpec[in.getNumColumns()+
    	                                               (m_settings.replaceColumn()?0:1)];
    	// iterate through the column specs and copy them to the new column specs
    	for (int i = 0; i < in.getNumColumns(); i++) {
            colSpecs[i] = in.getColumnSpec(i);
        }
    	 // if it is not to replace the column
        if (!m_settings.replaceColumn()) {
            colSpecs[colSpecs.length-1] = new DataColumnSpecCreator(
            		m_settings.appendColumnName(),CDKCell.TYPE).createSpec();
        }
        return colSpecs;
    }
