/*
 * Copyright (C) 2003-2013 The Jmol Development Team
 * 
...
/**
 * Provides an interface to CDK IO and CDK data classes. The <code>openBufferedReader</code> uses the ReaderFactory to
 * get an CDK Reader. The <code>getAtomIterator</code> and other methods that act on a <code>clientFile</code> accept
 * <code>AtomContainer</code>s and <code>ChemFile</code>s.
 */
public class CdkJmolAdapter extends JmolAdapter {
	...
	@Override
	public Object openBufferedReader(final String name, final BufferedReader bufferedReader) {
		...
		try {
			...
			chemFile = chemObjectReader.read(new org.openscience.cdk.ChemFile());
		...
		for (IAtomContainer ac : ChemFileManipulator.getAllAtomContainers(chemFile)) {
			for (IAtom atom : ac.atoms()) {
				try {
					...
					LOGGER.debug("  because: " + exception.getMessage() + exception);
				}
			}
		}
		return chemFile;
	...
	public String getAtomSetCollectionName(final Object clientFile) {
		...
		if (clientFile instanceof IChemObject) {
			Object title = ((IChemObject) clientFile).getProperty(CDKConstants.TITLE);
			if (title != null) {
	...
	public int getAtomSetCount(final Object clientFile) {
		...
		if (clientFile instanceof IAtomContainer) {
			...
			LOGGER.debug("Found #sets: " + ChemFileManipulator.getAllChemModels((IChemFile) clientFile).size());
	...
	public int getEstimatedAtomCount(final Object clientFile) {
		...
		LOGGER.debug("Don't know what kind of clientFile this is: " + clientFile.getClass().getName());
		return 0;
	}
	 ...
	 * 
	 * String[] getPdbStructureRecords(Object clientFile) { ChemFile chemFile = (ChemFile)clientFile; ChemSequence
	 * chemSequence = chemFile.getChemSequence(0); ChemModel chemModel = chemSequence.getChemModel(0); Vector
	 * structureVector = (Vector)chemModel.getProperty("pdb.structure.records"); if (structureVector == null) return
	 * null; String[] t = new String[structureVector.size()]; structureVector.copyInto(t); return t; }
	 */
	...
	public float[] getNotionalUnitcell(final Object clientFile) {
		...
		if (clientFile instanceof ICrystal) {
			...
			double[] notional = CrystalGeometryTools
					.cartesianToNotional(crystal.getA(), crystal.getB(), crystal.getC());
	...
	@Override
	public String getClientAtomStringProperty(final Object clientAtom, final String propertyName) {
	...
	@Override
	public JmolAdapter.StructureIterator getStructureIterator(final Object clientFile) {

		LOGGER.debug("Jmol requested a stucture iterator for a " + clientFile.getClass().getName());
		if (clientFile instanceof IChemFile) {
