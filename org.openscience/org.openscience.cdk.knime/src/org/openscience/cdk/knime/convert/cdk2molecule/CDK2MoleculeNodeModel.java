...
/**
 * This class is the model for the CDK->Molecule node. It converts CDK molecules
 * into different textual representations.
 *
...
public class CDK2MoleculeNodeModel extends NodeModel {
    ...
    protected DataTableSpec[] configure(final DataTableSpec[] inSpecs)
        ...
        if (m_settings.columnName() == null) {
                ...
                if (cs.getType().isCompatible(CDKValue.class)) {
                    if (m_settings.columnName() != null) {
                        setWarningMessage("Selected column '"
                                + m_settings.columnName() + "' as CDK column");
                    } else {
            ...
            if (!inSpecs[0].getColumnSpec(m_settings.columnName()).getType()
    ...
    private ColumnRearranger createRearranger(final DataTableSpec inSpec) {
        ...
        if (m_settings.replaceColumn()) {
            cs =
                    new DataColumnSpecCreator(m_settings.columnName(), type)
                            .createSpec();
        } else {
            String name =
        ...
        MolConverter conv =
                new MolConverter(cs, inSpec.findColumnIndex(m_settings
                        .columnName()));
    ...
    protected void validateSettings(final NodeSettingsRO settings)
        ...
        if (!s.replaceColumn() && s.newColumnName().length() < 1) {
            throw new InvalidSettingsException(
                    "No name for the new column entered");
        }
    }
}
