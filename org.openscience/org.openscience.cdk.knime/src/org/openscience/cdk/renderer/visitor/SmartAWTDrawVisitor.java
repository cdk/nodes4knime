...
public class SmartAWTDrawVisitor extends AbstractAWTDrawVisitor {
	...
	/**
	 * The padding used between child elements of a text element group.
	 */
	private static final int PADDING = 2;
	private static final int SUB_FONT = 3;

	/**
	 * The font manager cannot be set by the constructor as it needs to be
	 * managed by the Renderer.
	 */
	private AWTFontManager fontManager;

	/**
	 * The renderer model cannot be set by the constructor as it needs to be
	 * managed by the Renderer.
	 */
	 ...
	 * taking a {@link Graphics2D} object to which the chemical content is
	 * drawn.
	 * 
	 * @param graphics {@link Graphics2D} to which will be drawn
	 */
	public SmartAWTDrawVisitor(Graphics2D graphics) {
	...
	private void visit(LineElement line) {
		...
		// scale the stroke by zoom + scale (both included in the
		// AffineTransform)
		float width = (float) (line.width * transform.getScaleX());
		if (width < 1.5f)
			width = 1.5f;

		int key = (int) (width * 4); // store 2.25, 2.5, 2.75 etc to separate
										// keys
	...
	private void visit(RectangleElement rectangle) {
		...
		int[] point2 = this.transformPoint(rectangle.xCoord + rectangle.width, rectangle.yCoord + rectangle.height);
	...
	private void visit(SmartTextGroupElement textGroup) {
		this.graphics.setFont(this.fontManager.getFont());
		Point point = super.getTextBasePoint(textGroup.text, textGroup.xCoord, textGroup.yCoord, graphics);
		RoundRectangle2D textBounds = this.getTextBoundsRounded(textGroup.text, textGroup.xCoord, textGroup.yCoord,
				graphics);
		
		this.graphics.setColor(textGroup.getBackgroundColor());
        this.graphics.fill(textBounds);
        this.graphics.setColor(textGroup.color);
		this.graphics.setBackground(textGroup.getBackgroundColor());
        this.graphics.drawString(textGroup.text, point.x, point.y);

		Rectangle2D narrowTextBounds = this.getNarrowTextBounds(textGroup.text, textGroup.xCoord, textGroup.yCoord,
				graphics);
		int xCoord = (int) narrowTextBounds.getCenterX();
		int yCoord = (int) narrowTextBounds.getCenterY();
		int xCoord1 = (int) narrowTextBounds.getMinX();
		int yCoord1 = (int) narrowTextBounds.getMinY();
		int xCoord2 = point.x + (int) narrowTextBounds.getWidth();
		int yCoord2 = (int) narrowTextBounds.getMaxY();

		int oHeight = yCoord2 - yCoord1;
		for (SmartTextGroupElement.Child child : textGroup.children) {
			int childx;
			int childy;

			switch (child.position) {
			case NE:
				childx = xCoord2 + PADDING;
				childy = yCoord1;
				break;
			case N:
				childx = xCoord1;
				...
				break;
			case NW:
				double spacing = addjustWest(child);
				childx = (int) (xCoord1 - spacing);
				childy = yCoord1;
				break;
			case W:
				spacing = addjustWest(child);
				childx = (int) (xCoord1 - spacing - PADDING);
				childy = point.y;
				break;
			case SW:
				spacing = addjustWest(child);
			...
			case S:
				childx = xCoord1;
				childy = yCoord2 + oHeight + PADDING;
				break;
			case SE:
				childx = xCoord2 + PADDING;
			...
			if (child.text.equals("..")) {
				int dim = (int) (narrowTextBounds.getWidth() * 0.3);
				int childyCor = childy - oHeight / 3;
				if (child.position.name().contains("S")) {
					childyCor = childyCor - 2 * dim;
				}
				this.graphics.fillOval(childx, childyCor, dim, dim);
			} else {
				this.graphics.drawString(child.text, childx, childy);
			}
			if (child.subscript != null) {
				Rectangle2D childBounds = getNarrowTextBounds(child.text, childx, childy, graphics);
				int scx = (int) (childx + childBounds.getWidth());
				int scy = (int) (childy + (childBounds.getHeight() / 2));
				Font font = this.graphics.getFont();
				Font subscriptFont = font.deriveFont(font.getStyle(), font.getSize() - SUB_FONT);
				this.graphics.setFont(subscriptFont);
				this.graphics.drawString(child.subscript, scx + PADDING, scy);
			}
		}
	}
	...
	public void visit(IRenderingElement element) {
		...
		else if (element instanceof SmartTextGroupElement)
			visit((SmartTextGroupElement) element);
	...
	/**
	 * The font manager must be set by any renderer that uses this class! This
	 * manager is needed to keep track of fonts of the right size.
	 * 
	...
	public void setRendererModel(RendererModel rendererModel) {
		...
		if (rendererModel.hasParameter(UseAntiAliasing.class)) {
			if ((boolean) rendererModel.getParameter(UseAntiAliasing.class).getValue()) {
				...
				// g.setStroke(new
				// BasicStroke((int)rendererModel.getBondWidth()));
			}
		}
	}

	/**
	 * Calculates the narrow boundaries of a text string.
	 * 
	 * @param text the text string
	 * @param graphics the graphics
	 * @return the narrow bounds
	 */
	protected Rectangle2D getNarrowTextBounds(String text, double xCoord, double yCoord, Graphics2D graphics) {

		TextLayout layout;
		if (text.isEmpty()) {
			layout = new TextLayout(" ", graphics.getFont(), graphics.getFontRenderContext());
		} else {
			layout = new TextLayout(text, graphics.getFont(), graphics.getFontRenderContext());
		}
	 ...
	 */
	protected RoundRectangle2D getTextBoundsRounded(String text, double xCoord, double yCoord, Graphics2D graphics) {
		...
		double width = bounds.getWidth() + bounds.getHeight() * 0.8;
		double height = bounds.getHeight() + bounds.getHeight() * 0.8;
		boolean blunt = bounds.getWidth() / bounds.getHeight() > 3;

		int[] point = transformPoint(xCoord, yCoord);

		return new RoundRectangle2D.Double(point[0] - width / 2, point[1] - height / 2, width, height, blunt ? height
				: width, height);
	}

	private double addjustWest(SmartTextGroupElement.Child child) {

		Rectangle2D childBounds = getNarrowTextBounds(child.text, 0, 0, graphics);
		double subscript = 0;
		if (child.subscript != null && !child.subscript.isEmpty()) {
			Font font = this.graphics.getFont();
			Font subscriptFont = font.deriveFont(font.getStyle(), font.getSize() - SUB_FONT);
			this.graphics.setFont(subscriptFont);
			Rectangle2D subBounds = getNarrowTextBounds(child.subscript, 0, 0, graphics);
			this.graphics.setFont(this.fontManager.getFont());
			subscript += subBounds.getWidth();
		}

		double spacing = childBounds.getWidth() + subscript;
		return (spacing > 10) ? spacing + 1 : spacing;
	}
}
