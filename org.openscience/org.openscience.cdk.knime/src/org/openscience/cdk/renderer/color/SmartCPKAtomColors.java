...
public class SmartCPKAtomColors implements IAtomColorer, java.io.Serializable {
...
//	private static final Color WHITE = new Color(0xFFFFFF);
	 ...
	 * @return A color for the atom. The default colour is used if none is found
	 *         for the atom.
	 */
	...
	public Color getAtomColor(IAtom atom, Color defaultColor) {
		...
		if (atom.getAtomicNumber() != null && ATOM_COLORS_MASSNUM.containsKey(atom.getAtomicNumber())) {
			color = ATOM_COLORS_MASSNUM.get(atom.getAtomicNumber()); // lookup
																		// by
																		// atomic
																		// number.
		...
		return new Color(color.getRed(), color.getGreen(), color.getBlue()); // return
																				// atom
																				// copy.
	}
}
