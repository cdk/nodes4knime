...
public class SmartBasicBondGenerator implements IGenerator<IAtomContainer> {
    ...
    /**
	 * The bond width used when highlighted.
	 */
	private int BOND_HIGHLIGHT_WIDTH = 2;
     ...
     */
    public SmartBasicBondGenerator() {}
    ...
    public Color getColorForBond(IBond bond, RendererModel model) {
		...
		if (bond.getProperty(CDKConstants.ANNOTATIONS) != null) {
			color = new Color(bond.getProperty(CDKConstants.ANNOTATIONS, Integer.class));
		}
    ...
    public double getWidthForBond(IBond bond, RendererModel model) {
        ...
        } else if (bond.getProperty(CDKConstants.ANNOTATIONS) != null) {
            return BOND_HIGHLIGHT_WIDTH;
        } else {
