...
public class SmartALogPDescriptor extends AbstractMolecularDescriptor implements IMolecularDescriptor {
	...
	int[] frags = new int[121]; // counts of each type of fragment in the
								// molecule
	public int[] alogpfrag; // alogp fragments for each atom (used to see which
							// atoms have missing fragments)
	final static double[] fragval = new double[121];// coefficients for alogp
													// model
	static {
	...
	final static double[] refracval = new double[121]; // coefficients for
														// refractivity model
	static {
	...
	public SmartALogPDescriptor() throws CDKException {
		try {
	...
	private int getHAtomType(IAtom ai, java.util.List connectedAtoms) {
		...
		if (ai.getSymbol().equals("C") && !ai.getFlag(CDKConstants.ISAROMATIC)) {
			for (int j = 0; j <= ca.size() - 1; j++) {
				...
				} // end if (atomContainer.getBond(ai,
					// ((IAtom)ca.get(j))).getOrder() == IBond.Order.SINGLE) {
			}// end j loop
		...
		for (int j = 0; j <= bonds.size() - 1; j++) {
			if (((IBond) bonds.get(j)).getOrder() == IBond.Order.DOUBLE)
				doublebondcount++;
			else if (((IBond) bonds.get(j)).getOrder() == IBond.Order.TRIPLE)
				triplebondcount++;
		}
	...
	private void calcGroup059_060_063(int i) {
		...
		if (fragment[i].equals("SssO")) {
			if (ca0.getFlag(CDKConstants.ISAROMATIC) || ca1.getFlag(CDKConstants.ISAROMATIC)) {
				...
				for (int j = 0; j <= ca.size() - 1; j++) {
					// if (((IAtom)ca.get(j)).getSymbol().equals("C")) { // for
					// malathion
					// O-P(=S)
	...
	private void calcGroup107(int i) {
		...
		// IAtom []
		// ca=atomContainer.getConnectedAtoms(atomContainer.getAtomAt(i));
		//
